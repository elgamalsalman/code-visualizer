...
let users_dir = path.resolve(config.users.files.root_dir_path);
...
const get_user_dir_name = (user_id) => {
	return `user_${user_id}`;
};

const get_user_status = async (user_id) => {
	const user_dir_path = await get_user_dir_path(user_id);
...
const set_user_status_to_free = async (user_id) => {
	const free_status = config.file_manager.user_statuses.free_status;
	await set_user_status(user_id, free_status);
};

const set_user_status = async (user_id, status) => {
	const user_dir_path = await get_user_dir_path(user_id);
	const free_status = config.file_manager.user_statuses.free_status;
	const pending_statuses = Object.values(
		config.file_manager.user_statuses.pending_statuses
...
const wait_for_pending_user = (user_id) => {
	return new Promise((resolve, reject) => {
		const period = config.file_manager.user_statuses.pending_check_period;
		const free_status = config.file_manager.user_statuses.free_status;
		const interval = setInterval(async () => {
			if ((await get_user_status(user_id)) === free_status) {
...
const create_user_dir = async (user_id) => {
	const user_dir_path = path.resolve(users_dir, get_user_dir_name(user_id));
...
const update_user_files = async (user_id, updates) => {
	...
	try {
		...
		await set_user_status_to_free(user_id);
	}
};

const get_user_file_tree = async (user_id) => {
	// create user directory if it doesn't exist
	const user_dir_path = await get_user_dir_path(user_id);
...
const get_user_entities = async (user_id, entity_metas) => {
	// create user directory if it doesn't exist
	const user_dir_path = await get_user_dir_path(user_id);
