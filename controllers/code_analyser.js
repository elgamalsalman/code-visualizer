...
import { count_occurrences, get_nth_occurrence_index } from '../utils/container_utils.js';
import { tokenize } from '../utils/string_utils.js';

// controllers
import PTY from './pty_controller.js';
import GDB from './gdb_controller.js';

// TODO put this in a config file
// ---------- CONFIG ----------

const CODE_HEADER_FILE_PATH = './scripts/code_header.cpp';
const NAMED_PIPE_MAINTAINER_BASH_SCRIPT_FILE_PATH = './scripts/named_pipe_maintainer.sh';
...
class Code_Analyser {
	...
	#events_callbacks = {
		...
		"new": async (res) => {
			...
			// get class name
			const event_index = this.#get_event_cursor_index(line_number, line, event);
			...
			if (class_name in CLASSES) {
				...
				const object_address = await this.#gdb.examine_pointed_value(ptr_address, 'a');
				...
				for (const { name: property_name, type: property_type } of CLASSES[class_name].properties) {
					...
					const property_address = await this.#gdb.get_variable_address(`((${class_name}*)(${object_address})).${property_name}`);

					// setup a watchpoint
					res = await this.#gdb.execute(`watch (${property_type})(*(${property_address}))`);
					const watchpoint = GDB.get_controlpoint_number(res);
		...
		"delete": async (res) => {
			...
			res = await this.#gdb.execute('watch ptr');
			const watchpoint = GDB.get_controlpoint_number(res);
			res = await this.#gdb.execute('continue'); res.splice(0, 1);
			assert(watchpoint === GDB.get_controlpoint_number(res));
			await this.#gdb.execute(`delete ${watchpoint}`);
			const object_address = await this.#gdb.examine_pointed_value(ptr_address, 'a');
		...
		"change": (object_address, property_name, property_address, property_type) => {
			return async (res) => {
				...
				const new_value = await this.#gdb.examine_pointed_value(property_address, examination_format);
  ...
  constructor(code_file_path, events_handler) {
		...
		this.#code_file_name = code_file_path.substr(0, code_file_path.lastIndexOf("."));
		this.#executable_file_path = this.#code_file_name + '.' + EXECUTABLE_FILE_EXTENSION;
		this.#program_input_file_path = this.#code_file_name + '.' + INPUT_FILE_EXTENSION;
		this.#program_output_file_path = this.#code_file_name + '.' + OUTPUT_FILE_EXTENSION;
		this.#program_error_file_path = this.#code_file_name + '.' + ERROR_FILE_EXTENSION;
	...
	#add_tracked_object_controlpoint = (object_id, controlpoint) => {
		...
		assert(this.#tracked_objects_set.has(object_id) &&
	...
	#init = async () => {
		...
		await PTY.execute(`truncate`, [`-s`, `0`, `${this.#program_output_file_path}`]); // empty program output file
		await PTY.execute(`truncate`, [`-s`, `0`, `${this.#program_error_file_path}`]); // empty program error file

		// compile code
		await PTY.execute(`g++`, [
			`-g`, `${this.#code_header_file_path}`, `${this.#code_file_path}`, `-o`, `${this.#executable_file_path}`
		]);
		console.error(`[*] compiled`);

		// maintain the input named pipe opened
		const program_input_named_pipe_maintainer_process = new PTY('bash',
			[`${NAMED_PIPE_MAINTAINER_BASH_SCRIPT_FILE_PATH}`, `${this.#program_input_file_path}`],
		);

		// run code
		console.error(`[*] running...`);
		this.#gdb = new GDB(['-q', `${this.#executable_file_path}`]);
		this.#program_output_process = new PTY('tail', ['-f', `${this.#program_output_file_path}`]);
		this.#program_error_process = new PTY('tail', ['-f', `${this.#program_error_file_path}`]);
	...
	run = async () => {
		...
		// create breakpoints
		res = await this.#gdb.execute('break operator new'); console.error(res);
		this.#set_controlpoint_callback(GDB.get_controlpoint_number(res), this.#events_callbacks["new"]);
		res = await this.#gdb.execute('break operator delete(void*)'); console.error(res);
		this.#set_controlpoint_callback(GDB.get_controlpoint_number(res), this.#events_callbacks["delete"]);

		// run program
		res = await this.#gdb.execute(
			`run < ${this.#program_input_file_path} 1> ${this.#program_output_file_path} 2> ${this.#program_error_file_path}`
		);
		res.splice(0, 1); // remove starting line
		console.error(res);

		while (GDB.is_controlpoint(res)) {
			const controlpoint_callback = this.#get_controlpoint_callback(GDB.get_controlpoint_number(res));
